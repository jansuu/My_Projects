...
public class BookingServiceTest 
{
	...
	{
		when(bookingRepository.save(b2)).thenReturn(b1);
		assertEquals(b1, bookingService.createBooking(b2));
	}
	
	@Test
	public void getBookingsByCustomer() throws InvalidIDException 
	{
		List<Booking> booking = Arrays.asList(b1, b2);
		when(bookingRepository.findByCustomerId(c1.getId())).thenReturn(booking);
		assertEquals(booking, bookingService.getBookingsByCustomer(1));
	}
